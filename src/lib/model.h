...
struct ligand : public flexible_body, atom_range {
    unsigned degrees_of_freedom; // Can differ from the apparent number of rotatable bonds due to disabled torsions
...
public:
    // Constructors
    model() : m_num_movable_atoms(0), m_atom_typing_used(atom_type::XS), m_oxygen_level(1.0) {}
    model(atom_type::t atype) : m_num_movable_atoms(0), m_atom_typing_used(atype), m_oxygen_level(1.0) {}

    // Accessors for atom and coordinate data
    ...
    // New Getter and Setter for Drug Type
    void set_drug_type(const std::string& drug_type) { m_drug_type = drug_type; }
    std::string get_drug_type() const { return m_drug_type; }

    // New Getter and Setter for Oxygen Level
    void set_oxygen_level(fl oxygen_level) {
        if (oxygen_level < 0.0 || oxygen_level > 1.0) {
            throw std::invalid_argument("Oxygen level must be between 0.0 and 1.0");
        }
        m_oxygen_level = oxygen_level;
    }
    fl get_oxygen_level() const { return m_oxygen_level; }
...
private:
    ...
    // New member variables for drug type and oxygen level
    std::string m_drug_type; // Stores the drug type of the ligand
    fl m_oxygen_level;       // Stores the oxygen level (0.0 to 1.0)
};
